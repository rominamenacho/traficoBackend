...
public class VencimientoChoferCarnetNacional implements VencimientoChofer{    
    ...
    TipoCarnet tipoCarnet = TipoCarnet.NACIONAL;
    ...
    public List<ChoferDTO> calcularVencimiento(String empresa, int estadoChofer, Vencimiento vencimiento ) {      
        ...
        carnets.forEach( carnetNac ->{                
            Chofer chofer = carnetNac.getChofer();
            if( chofer != null ) {                   
                VencimientoCalculadoDTO calculado = calcularVencimientos(  carnetNac,  vencimiento );                
                if( calculado != null ){
                     ChoferDTO choferDTO = choferMapper.toDTO( chofer );
                     choferDTO.getVencimientos().add(  calcularVencimientos(  carnetNac,  vencimiento )  );
    ...
    public VencimientoCalculadoDTO calcularVencimientos( Carnet carnetNac, Vencimiento vencimiento ) {       

        if( carnetNac == null || carnetNac.getFechaEmision() == null ) {
                return null;
        }		

         GregorianCalendar vencimientoCarnet = new GregorianCalendar();
         vencimientoCarnet.setTime( carnetNac.getFechaVenc() );   		    
         ...
         calculado.setFechaVencimiento( carnetNac.getFechaVenc() );
